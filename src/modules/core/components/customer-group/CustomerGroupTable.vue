...
const save = async () => {
    try {
        busy.value = true;
        if (isEditMode.value) {
            await customerGroupStore.update(
                selectedItem.value.id,
                formData.value
            );
        } else {
            await customerGroupStore.create(formData.value);
        }
        closeDialog();
        await getItems();
        selectedItem.value = {};
...
const deleteItem = async () => {
    try {
        ...
        if (selectedItem.value) {
            await customerGroupStore.deleteItem(selectedItem.value.id);
        }
        await getItems();
        selectedItem.value = {};
    } finally {
...
const changeStatus = async () => {
    try {
        loading.value = true;
        if (selectedItem.value) {
            await customerGroupStore.changeStatus(selectedItem.value.id);
        }
        await getItems();
        selectedItem.value = {};
    } finally {
...
<template>
    <TitleHeader>
        <template #title>
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold">Customer Groups</h1>
                <p class="mt-2 text-sm sm:text-base mx-auto sm:mx-0 mb-0">
                    These customer groups are used when adding customers.
                </p>
            </div>
        </template>
        <template #actions>
    ...
    <Card class="py-3 px-2">
        <template #content>
            ...
            >
                ...
                <template #empty> No customer groups found. </template>
                <Column
                    class="w-[30rem]"
                    :sortable="true"
                    field="name"
    ...
    <BaseDialog
        ...
        :header="isEditMode ? 'Edit Customer Group' : 'New Customer Group'"
    ...
    <Confirmation
        ...
        :content="`Are you sure you want to make this customer group ${isItemActive ? 'inactive' : 'active'}?`"
