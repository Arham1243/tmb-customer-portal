...
const exportMenuItems = [
    {
        ...
        label: 'To Excel',
        command: () => exportData({ table: 'timesheets', format: 'excel' })
    },
    {
        icon: 'pi pi-file-pdf',
        label: 'To PDF',
        command: () => exportData({ table: 'timesheets', format: 'pdf' })
    }
];
...
const exportData = async ({ table, format }) => {
    try {
        loading.value = true;
        const columns = ['date', 'input_hours', 'status'];
        await exportTable({
            table,
            format,
...
const getItems = async () => {
    ...
    try {
        loading.value = true;
        const params = { ...pagination.getPageParams(), merge_by_week: true };
        const payload = {
